...
minList [] = -1
minList [ x ] = x
minList (x : xs) = verifySmallest x (minList xs)

verifySmallest :: Int -> Int -> Int
verifySmallest x y
     | x > y = y
     | otherwise = x
...
indexOf element [] = -1
indexOf element [a] = -1
indexOf element (x : xs)
     | element == x = 1
     | element /= x = 1 + indexOf element xs
     |  otherwise = -1
...
removeAll number (x:xs)
     | x == number = removeAll x xs
     | x /= number = x : removeAll number xs

removeAllWithListComprehension :: Int -> [Int] -> [Int]
removeAllWithListComprehension number [] = []
removeAllWithListComprehension number list = [ n | n <- list , n /= number ]
...
countPos :: [Int] -> Int
countPos [] = 0
countPos list = length [ x | x <- list, x >= 0 ]
