...
> inRange il sl list = [x | x <- list, x >= il && x <= sl]
...
> isInRange il sl [] = True
> isInRange il sl (x : xs) = x >= il && x <= sl && (isInRange il sl xs)

> inRangeProperty :: Int -> Int -> [Int] -> Bool
> inRangeProperty il sl [] = True
> inRangeProperty il sl list = isInRange il sl (inRange il sl list)
