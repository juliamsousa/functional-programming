...
\begin{code}
sumSquareNumbers :: Int -> Int -> Int -> Int
sumSquareNumbers x y z
     | x>=y && z>=y = x^2+z^2
     | x>=z && y>=z = x^2+y^2
     | otherwise = y^2+z^2
\end{code}
...
Essa resolucao eh baseada na forma canonica de se resolver fibonacci de forma pura e funcional sem recalculo
\begin{code}
linearFibonacci :: Int -> Int
-- recebe um parametro n e utiliza da lazyness para pegar o enesimo numero com o operador !!
linearFibonacci n = fibonacciList!!n
    -- fibonacciList eh uma lista infinita por se tratar de uma linked list e por haskell ser uma linguagem lazy
    -- pega os elementos 0 e 1 da listas ( 0: 1:)
    -- e soma com o restante da lista que eh representado por tail fibonacciList
    {-- 
      zipWith: funcao que faz uma operacao com listas, realizando a operacao especificada
      com os termos equivalentes da lista. nesse caso os elementos da lista serao somados (+)
      tail: funcao que retorna uma lista sem o seu primeiro elemento
      os termos equivalentes serao o sucessor e antecessor na sequencia de fibonacci, uma vez que
      um termo sempre eh retirado atraves do tail
    --}
    where fibonacciList = 0 : 1 : zipWith (+) fibonacciList (tail fibonacciList)
\end{code}